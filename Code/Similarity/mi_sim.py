...
# k-mer 的设定需要实验
def build_similarity_matrix(csv_file, k_circ=2, k_mi=5, output_file="F:\PyCharmProject\GECCMI\mydataset\similarity\mirna_sim_matrix."):
    """ 从 CSV 读取 RNA 序列，计算相似性矩阵，并保存为 CSV """
    df = pd.read_csv(csv_file)

    # 提取 RNA 名称和序列
    rna_names = df["miRNA"].tolist()
    rna_sequences = df["sequence"].tolist()

    # 计算 k-mer 频率
    kmer_dict = {name: kmer_count(seq, k_mi) for name, seq in zip(rna_names, rna_sequences)}
...
csv_path = "F:\PyCharmProject\GECCMI\dataop\myCMI\miRNA_seq.csv" #序列路径
